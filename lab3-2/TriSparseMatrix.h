...
// 稀疏矩阵三元组顺序表的函数成员：
    ...
    // operator+重载
    TriSparseMatrix<ElemType> operator+(const TriSparseMatrix<ElemType> &b) const;
...
template<class ElemType>
TriSparseMatrix<ElemType> TriSparseMatrix<ElemType>::operator+(const TriSparseMatrix<ElemType> &b) const {
    TriSparseMatrix<ElemType> c(rows, cols, maxSize + b.maxSize);    // 构造一个临时矩阵c
    int i = 0, j = 0, k = 0;                                        // 工作变量
    while (i < num && j < b.num) {
        if (triElems[i].row < b.triElems[j].row ||
            triElems[i].row == b.triElems[j].row && triElems[i].col < b.triElems[j].col) {
            c.triElems[k++] = triElems[i++];                        // 将当前三元组插入c中
        } else if (triElems[i].row == b.triElems[j].row &&
                   triElems[i].col == b.triElems[j].col) {
            ElemType v = triElems[i].value + b.triElems[j].value;    // 计算当前三元组的值
            if (v != 0) {
                c.triElems[k].row = triElems[i].row;                // 行
                c.triElems[k].col = triElems[i].col;                // 列
                c.triElems[k++].value = v;                            // 非零元素值
            }
            i++;
            j++;
        } else {
            c.triElems[k++] = b.triElems[j++];                        // 将当前三元组插入c中
        }
    }
