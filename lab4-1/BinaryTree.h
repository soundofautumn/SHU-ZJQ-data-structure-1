...
// 求以r为根的二叉树的结点个数
    ...
    int LeafCount(const BinTreeNode<ElemType> *r) const;    // 求以r为根的二叉树的叶子结点个数
public:
//  二叉树方法声明及重载编译系统默认方法声明:
    BinaryTree();                        // 无参数的构造函数
    explicit BinaryTree(const ElemType &e);        // 构造以e为根的二叉树
    ...
    //实现非递归的二叉树中序遍历算法
    void InOrderWithStack(void (*Visit)(const ElemType &)) const;
...
template<class ElemType>
void BinaryTree<ElemType>::InOrderWithStack(void (*Visit)(const ElemType &)) const {
    // 操作结果：实现非递归的二叉树中序遍历算法
    LinkStack<BinTreeNode<ElemType> *> s;    // 定义栈s
    BinTreeNode<ElemType> *p = root;

    while (p != NULL || !s.IsEmpty()) {    // p非空或栈非空
        while (p != NULL) {                // p非空
            s.Push(p);                    // p入栈
            p = p->leftChild;            // p移至左孩子
        }
        if (!s.IsEmpty()) {                // 栈非空
            s.Pop(p);                    // p退栈
            (*Visit)(p->data);            // 访问p结点
            p = p->rightChild;            // p移至右孩子
        }
    }
}

template<class ElemType>
bool BinaryTree<ElemType>::IsCompleteBinaryTree() const {
    // 操作结果：判断二叉树是否为完全二叉树
    LinkQueue<BinTreeNode<ElemType> *> q;    // 定义队列q
    BinTreeNode<ElemType> *p;
        ...
        if (p->leftChild != NULL)            // 队头元素左孩子非空
            ...
            while (!q.IsEmpty()) {            // 队列非空
                ...
                if (p->leftChild != NULL || p->rightChild != NULL)
                    return false;            // 队头元素有孩子结点，不是完全二叉树
            }
            return true;                    // 队列为空，是完全二叉树
        }
        if (p->rightChild != NULL)            // 队头元素右孩子非空
            q.EnQueue(p->rightChild);        // 右孩子入队
        else {                                // 队头元素右孩子为空
