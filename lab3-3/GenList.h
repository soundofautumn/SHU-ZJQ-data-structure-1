...
// 辅助函数
    ...
    void ReverseHelp(GenListNode<ElemType> *&hd);
...
// 抽象数据类型方法声明及重载编译系统默认方法声明:
    ...
    Status Insert(const ElemType &e, int i);    // 将原子元素e作为表头加入到广义表第i个元素之前
    Status Insert(GenList<ElemType> &subList, int i); // 将子表subList作为表头加入到广义表第i个元素之前

    void Reverse();// 逆置广义表

    GenList<ElemType> GetHead() const;    // 返回广义表的头结点
    GenList<ElemType> GetTail() const;    // 返回广义表的尾子表
...
template<class ElemType>
Status GenList<ElemType>::Insert(const ElemType &e, int i) {
    if (i < 1 || i > GetLength() + 1)
        return RANGE_ERROR;        // 返回位置错
    else {
        GenListNode<ElemType> *pre = head;
        for (int k = 1; k < i; k++)    // 使pre指向被插入位置的前驱结点
            pre = pre->tLink;
        GenListNode<ElemType> *p = new GenListNode<ElemType>(ATOM, pre->tLink);
        p->atom = e;                // 数据域
        pre->tLink = p;            // 将p插入在pre与pre->tLink之间
        return SUCCESS;            // 返回插入成功
    }
}

template<class ElemType>
Status GenList<ElemType>::Insert(GenList<ElemType> &subList, int i) {
    if (i < 1 || i > GetLength() + 1)
        return RANGE_ERROR;        // 返回位置错
    else {
        GenListNode<ElemType> *pre = head;
        for (int k = 1; k < i; k++)    // 使pre指向被插入位置的前驱结点
            pre = pre->tLink;
        GenListNode<ElemType> *p = new GenListNode<ElemType>(LIST, pre->tLink);
        p->hLink = subList.head;    // 子表
        subList.head->ref++;        // subList引用数自加1
        pre->tLink = p;            // 将p插入在pre与pre->tLink之间
        return SUCCESS;            // 返回插入成功
...
template<class ElemType>
void GenList<ElemType>::Reverse() {
    // 递归反转广义表
    GenListNode<ElemType> *p = head->tLink;    // 临时变量
    head->tLink = NULL;                        // 置空广义表
    while (p) {
        if (p->tag == LIST)                    // 子表
            ReverseHelp(p->hLink);            // 递归反转子表
        GenListNode<ElemType> *q = p->tLink;    // 临时变量
        p->tLink = head->tLink;                // 将p插入在head与head->tLink之间
        head->tLink = p;
        p = q;
    }
}
...
template<class ElemType>
GenList<ElemType> GenList<ElemType>::GetHead() const {
    auto *subList = new GenList<ElemType>();
    if (head->tLink == NULL) return *subList;
    if (head->tLink->tag == ATOM) {
        subList->head = new GenListNode<ElemType>(HEAD);
        subList->head->ref = 1;
        subList->head->tLink = new GenListNode<ElemType>(ATOM);
        subList->head->tLink->atom = head->tLink->atom;
        return *subList;
    } else {
        subList->head = head->tLink->hLink;
        return *subList;
    }
}

template<class ElemType>
GenList<ElemType> GenList<ElemType>::GetTail() const {
    auto *subList = new GenList<ElemType>();
    subList->head = new GenListNode<ElemType>(HEAD);
    subList->head->ref = 1;
    subList->head->tLink = head->tLink->tLink;
    return *subList;
}
...
// 操作结果：显示以hd为头结点的广义表
{
    bool frist = true;
    cout << "(";                        // 广义表以(开始
    for (GenListNode<ElemType> *p = hd->tLink; p != NULL; p = p->tLink) {    // 依次处理广义表各元素
...
// 操作结果：返回以hd为表头的广义表的深度
{
    ...
    for (GenListNode<ElemType> *p = hd->tLink; p != NULL; p = p->tLink) {    // 求子表的最大深度
        if (p->tag == LIST) {    // 子表
...
template<class ElemType>
void GenList<ElemType>::ReverseHelp(GenListNode<ElemType> *&hd) {
    if (hd == NULL || hd->tLink == NULL) return;    // 空表或只有一个元素
    GenListNode<ElemType> *p = hd->tLink;    // 临时变量
    hd->tLink = NULL;                        // 置空广义表
    while (p) {
        if (p->tag == LIST)                    // 子表
            ReverseHelp(p->hLink);            // 递归反转子表
        GenListNode<ElemType> *q = p->tLink;    // 临时变量
        p->tLink = hd->tLink;                // 将p插入在head与head->tLink之间
        hd->tLink = p;
        p = q;
    }
}
